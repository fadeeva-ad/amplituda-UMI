<?php
	...
	class DemomarketPhpExtension extends ViewPhpExtension {
		...
		public function getCatagories(){
			$cats = new selector('pages');
			$cats->types('hierarchy-type')->name('content', 'page');
			$cats->where('hierarchy')->page(1);
			$cats->order('ord');
			$cats->group('obj_id');
			$cats->option('load-all-props')->value(true);
			$cats->option('no-length', true);
			return $cats->result();
		}
		public function getProducts($parent) {
			$products = new selector('pages');
			$products->types('hierarchy-type')->name('content', 'page');
			$products->where('hierarchy')->page($parent);
			$products->order('ord');
	...
	public function getPress() {
		$press = new selector('pages');
		$press->types('hierarchy-type')->name('content', 'page');
		$press->where('hierarchy')->page(5);
		$press->order('ord');
		$press->group('obj_id');
		$press->option('load-all-props')->value(true);
		$press->option('no-length', true);
		return $press->result();
	}
		...
		public function reviews($product = null,$limit = 0) {
			
			$reviews = new selector('pages');
			$reviews->types('object-type')->id(187);
			$reviews->types('hierarchy-type')->name('content', 'page');
			if($product) 
				$reviews->where('tovar')->equals($product->id);
			if($limit)
				$reviews->limit(0, $limit);
			$reviews->order('id')->desc();
			$reviews->group('obj_id');
			$reviews->option('load-all-props')->value(true);
			$reviews->option('no-length', true);
		...
		public function getCategoryList() {
			...
			foreach ($data['items'] as &$category) {
				...
				$category['alt'] = isset($category['alt_name']) ? $category['alt_name'] : '';
			}
		...
		public function getCollections($id) {
			...
			$sel->types('object-type')->id(190);
			$sel->where('brend')->equals($id);
		...
		public function getThumbnail(array $variables) {
			...
			$variables['class'] = isset($variables['class']) ? $variables['class'] : '';
			...
			$thumbnail['class'] = $variables['class'];
			return $thumbnail;
		}
		...
		public function isCheckStock() {
			return (bool) $this->requestSettingsContainer()
				->getValue('is_check_stock');
		}
		...
		public function isRootCatalog(array $variables) {
			...
			return ($variables['pageId']==229)?1:0;
		}
		...
		public function getCatalog(array $variables) {
			...
			if($variables['pageId']==229||$variables['pageId']==321){
			$template = null;
			$categoryId = 321;
			$limit = null;
			$ignorePaging = false;
			...
			return $this->macros(
				...
				$b_or_c = $variables['page']->getObjectId();
				$sel = new selector('pages');
				$sel->types('hierarchy-type')->name('catalog', 'object');
				$sel->where('brand')->equals($b_or_c);
				$sel->where('collection')->equals($b_or_c);
				$sel->option('or-mode','all');
		...
		public function getSmartFilters(array $variables) {
			$template = null;
			$categoryId = 321;//$variables['pageId'];
			$isAdaptive = false;
			$level = 2;
		...
		public function getComparedValueList(array $fieldData) {
			...
			foreach ($valueDataList as $valueData) {
				...
				$row[] = $propertyData['value'];
			}

			return array_slice($row, 0, self::MAX_PRODUCT_COUNT_FOR_COMPARISON);
		}
		...
		public function isShowCommentsOnProductPage() {
			return (bool) $this->requestSettingsContainer()
				->getValue('is_show_product_comments');
		}
		...
		public function isShowRatingOnProductPage() {
			return (bool) $this->requestSettingsContainer()
				->getValue('is_show_product_rating');
		}
		...
		public function getBanner() {
			if (!cmsController::getInstance()->isModule('banners')) {
				return [];
			}
